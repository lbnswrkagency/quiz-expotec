...
const ANALYTICS_API = 'http://localhost:5001/api/data';
...
const analytics = {
  ...
  updateQuizData: async function(quizId, isCorrect) {
    ...
    try {
      const response = await axios.put(`${ANALYTICS_API}/updateQuizData`, { quizId, isCorrect });
  ...
  incrementParticipants: async function(quizId) {
    try {
      const response = await axios.post(`${ANALYTICS_API}/incrementParticipants`, { quizId });
      return response.data;
    } catch (error) {
